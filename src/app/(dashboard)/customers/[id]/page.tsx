...
import { mockCustomerDetails, type MockCustomerOrder } from "@/_lib/mock-data";
...
const orderColumns: DataTableColumn<MockCustomerOrder>[] = [
  {
    id: "id",
    header: "O.S.",
    cell: (row) => (
...
export default async function CustomerProfilePage({ params }: Props) {
  ...
  const customer = mockCustomerDetails.find((c) => c.id === id);
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-label-md text-on-surface-variant mt-1 font-mono">
            ID: {customer.id} · {customer.visits} visitas
          </p>
        </div>
      ...
      <section className="space-y-4">
        <h2 className="text-label-lg text-on-surface font-mono tracking-wider uppercase">
          Histórico de Ordens de Serviço ({customer.orders.length})
        </h2>
      ...
      {customer.nextServices.length > 0 && (
        <section className="space-y-4">
          <h2 className="text-label-lg text-on-surface font-mono tracking-wider uppercase">
            Próximas Manutenções Programadas
          </h2>
          <div className="border-outline-variant bg-surface-container overflow-hidden rounded-lg border p-6">
            <ServiceTimeline
              nodes={customer.nextServices}
